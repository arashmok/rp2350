...
# - picotool (USB-enabled)
# - Raspberry Pi OpenOCD fork (RP2350 targets) with internal jimTCL
# - udev rules for Debug Probe + dialout/plugdev groups
...
# ---------- 0) Base packages ----------
banner "Installing base packages (build tools, cmake, ninja, libusb, pkg-config, python, serial utils)"
...
# Autotools + extras needed by OpenOCD bootstrap/configure
banner "Installing autotools and dev libs for OpenOCD (jimTCL/internal build path)"
sudo apt-get install -y \
...
cmake .. -DPICO_SDK_PATH="${PICO_SDK_DIR}" -DCMAKE_BUILD_TYPE=Release
cmake --build . -j
popd >/dev/null
...
if ! "${PICOTOOL_BIN}" --help | grep -q "load"; then
  ...
  echo "Ensure libusb-1.0-0-dev and pkg-config were installed, then rebuild:"
  echo "  rm -rf ${PICOTOOL_DIR}/build && mkdir ${PICOTOOL_DIR}/build && cd ${PICOTOOL_DIR}/build"
  echo "  cmake .. -DPICO_SDK_PATH=${PICO_SDK_DIR} -DCMAKE_BUILD_TYPE=Release && cmake --build . -j"
fi
...
# pull submodules so bundled jimtcl is available
git submodule update --init --recursive
./bootstrap
# use internal jimtcl to avoid pkg-config dependency on system jimtcl
...
# ---------- 5) udev rules + groups ----------
banner "Installing udev rules (Debug Probe & OpenOCD) and adding user to groups"
...
getent group plugdev >/dev/null 2>&1 && sudo usermod -aG plugdev "$USER" || true
...
echo "  - Build your project or pico-examples with:"
echo "      Arm   : cmake .. -DPICO_SDK_PATH=\$PICO_SDK_PATH -DPICO_PLATFORM=rp2350       -DPICO_BOARD=pico2"
echo "      RISC-V: cmake .. -DPICO_SDK_PATH=\$PICO_SDK_PATH -DPICO_PLATFORM=rp2350-riscv -DPICO_BOARD=pico2"
